...
// Aufruf der Batch-Datei upload.bat
void startBatchFile(const std::string& comPort) {
    std::string command = "upload.bat " + comPort;
...
int main() {
    ...
    // Starten der Batch-Datei mit dem COM-Port als Parameter
    startBatchFile(comPort);
    ...
    */
    ...
    while (true) {
        ...
        std::cout << "Ergebnis: " << result << std::endl;  // Gibt das Ergebnis aus

        logFile << "Benutzereingabe: " << input << " Ergebnis: " << result << std::endl;  // Schreibt die Kommunikation in die Logdatei
    }
